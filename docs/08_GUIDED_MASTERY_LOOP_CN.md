...
必须引用用户的真实行为或已掌握内容，并把一次完成或一次明确的修正表达为“小胜利”。即使回答不正确，也可以肯定“完成了可检查的尝试”或“明确暴露了具体困难”，但不得给予无依据积分、等级或夸奖。

`next_micro_action` 只能选择当前概念内的下一次练习、提示或补救，不能改变概念顺序、请求补充材料或结束会话。需要全局转换时，先生成 Evidence，再交给 Agent。

学习者界面不逐项显示以上字段。Quiz 只合成为：总分、三条逐题正确 / 需要复习、所选答案、正确答案（答错时）、简短原因和一个 Keep going；自由复述只保留结果、关键修正和 Keep going。`next_micro_action`、误解标签和 Evidence 字段留在后台供掌握循环与 Agent 使用。
...
`LearningEvidence` 不设默认学习者审阅页。用户从反馈选择 Keep going 时，系统在后台完成 Evidence 边界、请求 Agent 的一个下一动作并自动应用安全动作。

`source_gap_signal` 只能描述材料未覆盖的已命名内容，是观察而不是补充材料建议。
