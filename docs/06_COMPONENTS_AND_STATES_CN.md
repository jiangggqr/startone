...
支持当前、已完成、可进入、锁定和插入前置知识状态。节点之间用可见连接线表达路线或依赖；每个节点是键盘可操作按钮，点击后显示定义、作用和前置关系。不能只用颜色表达，也不能用节点点击绕过 Agent 路线判断。

### ConceptConnection

在当前讲解中只显示“前置概念 → 当前概念 → 下一概念”的轻量关系图，并用一句 Memory anchor 压缩核心定义。它帮助定向和记忆，不改变路线。
...
`pasted` 是介质类型，在 SourceBadge 中映射为 `uploaded`。AI 生成的改写或讲解必须声明“Generated from your material”，但它不是第二种学习来源。
...
后台保持固定结构化输出；用户界面按题型压缩。Quiz 显示总分、三条逐题答案核对、简短原因和 Keep going。
...
### AgentDecisionCard（内部状态）

保存一个推荐动作、理由和预计时间；普通安全动作不形成独立学习者页面，而是自动执行。生产学习界面不显示替代路径、Radio 或覆盖理由输入框；`request_more_material` 保留当前概念并在现有上传区显示简短提示。
