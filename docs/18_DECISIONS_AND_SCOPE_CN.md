...
- Agent 仍只选择一个下一动作；安全动作自动执行，减少确认和选择成本
- 已验证缺口只能触发 `request_more_material`，保留当前概念并打开上传区；网络搜索不属于产品能力

### 主要内容来源

用户上传或粘贴的材料是唯一学习入口和唯一学习来源。不提供“仅输入主题”生成学习内容的产品路径，不搜索网络，也不从外部知识库自动补课。
...
- 会话时长使用产品内部的紧凑默认值并允许随时暂停；可访问性和时间显示属于可选 Settings，不阻塞开始
...
- Feedback 的 `next_micro_action` 只允许在当前 Guided Mastery Loop 内选择一次局部练习；不得改变概念、路线、请求补充材料或结束会话
...
- `source_gap_signal` 只描述“材料没有覆盖某个已命名内容”的观察，不等于 `material_upload_needed`

### 材料缺口

网络搜索不属于 StartOne。命名 `SourceGap` 只有同时满足以下条件时才能影响下一步：

1. 缺口由服务端根据上传材料覆盖验证
2. 已验证 `LearningEvidence` 表明它阻断当前概念
3. Agent 的唯一动作是 `request_more_material`

接受该动作后保持当前概念，使用 `required_tool=open_material_upload` 打开现有上传区并说明缺少什么；不出现独立确认页。用户可上传相关材料，也可继续当前范围。任何模型调用都不获得网络工具。

### 用户控制

Agent 每次只选择一个动作。普通安全动作自动执行，不显示确认页、替代路径或覆盖理由输入框，以免重新制造决策成本；暂停、提示、重试和结束不视为失败。

`request_more_material` 是唯一会转回上传区的 Agent 动作：无经过验证且阻断学习的命名缺口时不得选择；执行后保持原概念和恢复点。无可验证前置关系时不得选择插入前置知识。
...
- 删除会话级联删除活动、尝试、Evidence、决策、材料缺口和该会话独占来源
...
生产 Web App 中所有用户可见内容统一使用英文，包括导航、按钮、来源标签、状态、错误、恢复提示、Tutor、练习、反馈、Agent 理由、材料缺口提示和数据控制。中文只用于内部产品规格与开发说明，不得出现在运行中的产品界面。Devpost 文案、README、演示视频音频或字幕也使用英文。
...
使用 GPT-5.6 与 Responses API。固定 UI 数据使用 Structured Outputs；Agent 使用受控工具/function calling；任何调用都不使用网络搜索工具。

- Milestone 0–1 不调用模型
- Milestone 2 首次建立真实 GPT-5.6 Structured Outputs 路径，用于来源覆盖、知识地图和初学者可读的概念解释，同时保留确定性 Demo 路径
- Milestone 4–6 复用同一服务端 AI gateway 实现 Tutor、练习、反馈、Agent 和材料缺口恢复
...
用户界面学习来源类型固定为 `uploaded`。`pasted` 是 `SourceDocument.media_kind`，在 UI 中映射为 `uploaded`。AI 生成的讲解显示“Generated from your material”，但不是第二种来源。

- PDF 定位：页码、页内片段序号、页内字符范围
- Markdown/TXT 定位：标题路径、开始/结束行、文档字符范围
- 粘贴文本定位：标题、段落序号、文档字符范围
- AI 生成的改写没有虚假页码或行号，必须带生成说明和它所依据的已验证上传来源引用
...
材料缺口验收夹具使用单独场景，只导入 `transformer_notes.md`，因此矩阵/点积前置解释成为命名缺口，并验证 `request_more_material` 打开上传区。低保真原型统一使用 Markdown 标题与行号；PDF 页码仅在独立 PDF 解析验收 fixture 中展示。

确定性夹具、材料缺口测试和运行模式只用于自动化或本地验收，不进入最终学习界面。真实产品由服务端 GPT-5.6 分析用户上传的材料；不得用固定结果处理任意公开上传。
...
- 导出、AI 活动、学习/无障碍偏好与删除全部数据进入 `Settings`；系统边界在来源标签、Agent 决策和材料缺口等相关时刻按上下文展示
...
- 上传材料中的说明显示 `uploaded`；AI 生成的示例必须严格由已验证材料支持并显示“Generated from your material”
- 真实 Tutor 调用不提供任何工具；只选取当前概念来源和与本次问题匹配的少量片段，不默认发送全部材料
...
- Milestone 4B 的提交只持久化原始尝试、提示深度和用时，不生成 `LearningEvidence`、Agent 决策或补充材料请求；这些尝试由 Milestone 4C 的统一反馈边界评估
...
- `LearningEvidence` 数据表和公开结构只保存活动类型、概念、结果、要点覆盖、误解标签、提示深度、用时、Tutor 困惑信号、补救结果、来源缺口信号和时间；明确不设 `next_action`、`recommendation`、`should_continue` 或 `material_upload_needed` 字段
...
- Milestone 4C 不创建 Agent 决策，不提供网络工具，也不请求补充材料；`evidence_ready` 只表示下一阶段可以由 Agent 读取证据

### 补充上传材料

- 请求必须绑定当前 session、concept、已验证 `SourceGap` 与 Agent 唯一选择的 `request_more_material` decision；不能由 Tutor、反馈或客户端自由制造缺口
- 接受后保持当前概念和返回点，使用 `required_tool=open_material_upload` 打开现有上传区，不增加独立确认页
- 页面只显示缺少什么、为什么影响当前概念、上传材料主动作和继续当前范围次动作
- 新材料沿用普通解析、位置验证和 workspace 所有权规则；解析后重新验证覆盖并回到原概念
- 用户选择继续当前范围或上传失败时不终止学习会话，也不立刻重复同一请求
- 所有真实和确定性调用都不提供网络工具；不存在外部候选结果或外部来源记录
