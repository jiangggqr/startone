...
展示固定 3 道单选题的 Quiz、按需原位出现的提示，以及“总分—逐题答案核对—为什么—Continue”的简短反馈；同时点开知识框架中的不同概念节点。

### 2:00–2:25 补救与 Agent

点击 Continue，短暂显示正在准备下一步，然后显示 Agent 选出的一个简洁动作。LearningEvidence 和 Agent 边界继续由后台与自动化测试验证，不当作学习内容展示。
