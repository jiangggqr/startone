...
- Treats uploaded material as the only learning source and preserves real page, heading, line, paragraph, and character locations.
- Uses GPT-5.6 to select a learning focus from that material and build source coverage, named gaps, a connected 2–5 concept knowledge map, and beginner-friendly explanations with relationship diagrams, key parts, examples, and memory anchors.
...
- Never searches the web. If a validated gap blocks learning, the Agent can only ask the learner to upload relevant material or continue within the current scope.
...
The human product decisions remained explicit: uploaded material as the only learning source; AI-selected learning focus without an upfront setup form; the Tutor/Guided Mastery/Agent responsibility boundary; observation-only `LearningEvidence`; named gaps that ask for another upload instead of searching; flat, English-only UI; and anonymous workspace privacy. When browser testing exposed reversed Tutor message order, clipped mobile headers, hidden learning features, unnecessary evaluator UI, and provenance relabeling risk, Codex corrected the implementation and added regression coverage.

## Challenges

The hardest challenge was not generating content; it was preventing responsibilities from leaking across boundaries. Tutor had to guide without changing the route. Feedback could recommend one local micro-action but could not make a global planning decision. The Agent had to reason from validated evidence without teaching or scoring. Missing coverage had to stop at a transparent request for learner-supplied material rather than silently introducing outside knowledge.
...
- Strict uploaded-source provenance from ingestion through Tutor, activities, feedback, and Agent decisions.
...
- An uploaded-material-only architecture in which no learning call receives a network tool.
...
- Public YouTube demo: https://youtu.be/S0k6eWLZzAY
