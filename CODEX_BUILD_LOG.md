...
> Final scope decision, 2026-07-22: uploaded or pasted material is the only learning source. The earlier external-retrieval experiment recorded in historical milestones below was removed from the product, UI, model tools and submission story. A blocking validated gap can only produce `request_more_material` with `required_tool=open_material_upload`.
...
- Decisions resolved at the baseline: Evidence-only learning judgment; local vs global next action; anonymous workspace isolation; file storage/delete/export; source location schemes; pause/conflict recovery; deterministic fixtures; Milestone 2 real-model entry. The earlier retrieval proposal was later removed by the final scope decision above.
...
- Remaining limitations: the environment has no `OPENAI_API_KEY`, so the live GPT-5.6 smoke test is deferred to final hardening; the start-action answer is currently restored from local storage and becomes a versioned server draft in Milestone 3; no network tool is exposed
...
- Verification: 21 automated tests passed; Demo quick actions, free questions, confusion signals, covered prerequisites, generated-example disclosures, pause locks, close/reopen persistence and optimistic thread versions passed; a fake Responses client verified `gpt-5.6`, typed `TutorResponseOutput`, `store=False` and absence of any `tools` argument; Python/JavaScript/diff checks passed; application files contained no Chinese UI strings or gradient declarations
- Browser verification: opened Tutor from the dominant focus action; ran a checking question; autosaved and submitted a free confusion message; displayed a factual signal with explicit non-Agent wording; generated an example from uploaded material with the correct disclosure; opened an exact source and returned focus to the invoking citation; paused/resumed the same conversation; closed Tutor and restored focus to its trigger
...
- Human product decisions: answers and scoring rubrics remain hidden until feedback; a 4B submission records facts but creates no `LearningEvidence`, Agent decision or material-upload request; Tutor remains the dominant recommended focus action while Quiz and recall are visually secondary
...
- Remaining limitations: the Adaptive Planning Agent is intentionally not created until Milestone 5; material-gap recovery remains unavailable until Milestone 6; the live GPT-5.6 smoke test still requires a server-side deployment key
...
- Goal: select and apply exactly one bounded global next action from validated evidence without letting the Agent teach, score, write arbitrary state or access outside information
- Codex contribution: added schema version 8; the eight-action Agent enum; immutable evidence snapshots; deterministic Demo policy; strict GPT-5.6 function calling; server validation of action, target and required tool; idempotent decisions; loop guards; penalty-free override; controlled concept activation; prerequisite/review detours and returns; and durable Agent/material-upload/session-summary states
- Human product decisions: `LearningEvidence` is the sole source for learning-performance claims; time, energy, route, prerequisites, coverage and retry history are feasibility constraints only; one recommendation is visually dominant; user overrides are recorded without penalty; `request_more_material` preserves the current concept and opens upload
...
- Verification: 32 automated tests passed; normal progression, retry, invalid override rejection, penalty-free finish, prerequisite insertion/return, named-gap action gating, evidence-based idempotency and workspace ownership passed; a fake Responses client verified `gpt-5.6`, `store=False`, a forced named function, `parallel_tool_calls=False`, strict schema and required fields; Python compilation, JavaScript syntax, diff hygiene, English-only UI scan and no-gradient scan passed
...
- Remaining limitations: material-gap recovery begins in Milestone 6; the live GPT-5.6 smoke test remains pending a server-side deployment key

## Milestone 6 — Historical retrieval experiment, superseded by material-gap recovery

- Status: superseded and removed from the final product on 2026-07-22
- Date: 2026-07-21
- Codex thread/session: current StartFrame Agent task
- Git commit: Milestone 6 focused commit (this commit)
- Historical outcome: a network-retrieval branch was implemented and tested, then rejected after learner review because it distracted from the core learning loop and contradicted the desired uploaded-material-only boundary.
- Final correction: the branch, permission settings, learner views, external-source records and model network tool were removed. The retained behavior is `request_more_material` with `required_tool=open_material_upload`; it preserves the current concept, opens the normal upload area and permits a no-penalty continue-current-scope choice.
- Verification target after correction: no learning call receives a network tool; the one-source fixture validates the named gap, the bounded upload request, parse recovery and return to the original concept.
...
- Codex contribution: added safe session copy/delete, full learning-record exports, AI activity visibility, workspace deletion, durable summaries, source-location reports, history filtering, saved accessibility/new-session preferences and hardened response headers. The historical topic-only fallback was later removed.
...
- Browser verification: the historical topic-only fallback was later removed; source reporting bound the exact uploaded chunk and restored focus; history filtering returned an exact match count; accessibility preferences persisted; data controls and deletion dialogs were exercised; summary recovery displayed completed/remaining concepts and a concrete restart action
...
- Remaining external publication gates: Git hosting publication and judge access; public HTTPS Demo; under-three-minute public YouTube video; replacement of pending URLs; user-confirmed Devpost submission before the deadline. The required real GPT-5.6 core smoke gate is complete.
...
- Recovery and trust: file status remains in the upload panel, all source references stay validated, `LearningEvidence` remains recommendation-free, the Agent remains exactly-one-action, and uploaded material remains the only source
...
- Implementation: added a versioned direct-start endpoint; starts the first route concept without a draft or baseline answer; concept map outputs now carry teaching key points and a concrete example; legacy saved maps receive a grounded source-based explanation fallback; upload and focus UI were simplified; Tutor, Quiz, recall, feedback, LearningEvidence and Agent boundaries remain unchanged
...
- Boundary preservation: map exploration never bypasses Agent-controlled progression; answer keys remain server-side; `LearningEvidence` remains observations only; the Agent still returns exactly one next action; uploaded material remains the only source
...
- Information reduction: removed the learner source-coverage page and the ordinary Agent decision page; coverage, candidate gaps, LearningEvidence and Agent action validation remain enforced server-side, while a blocking named gap only reopens the upload area
...
- Boundary preservation: uploaded material is the only learning source; Tutor explains and guides the current concept; Quiz, recall, hints, feedback, encouragement and remediation stay inside Guided Mastery; LearningEvidence contains observations only; Agent returns one action
- Verification: 51 automated tests passed after the final uploaded-material-only migration; Python compilation, JavaScript syntax, diff hygiene, English-only UI and no-gradient scans passed
